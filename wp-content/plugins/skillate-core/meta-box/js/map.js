(function ( $ ) {
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$skillate_core = this.$container.find( '.rwmb-map-skillate-core' );
			this.skillateprocore = this.$skillate_core[0];
		...
		initMapElements: function () {
			var defaultLoc = this.$skillate_core.data( 'default-loc' ),
				latLng;
			...
			this.map = new google.maps.Map( this.skillateprocore, {
				center: latLng,
				zoom: 14,
				streetViewControl: 0,
			...
			$address.autocomplete( {
				source: function ( request, response ) {
					var options = {
						...
						'region': that.$skillate_core.data( 'region' )
					};
